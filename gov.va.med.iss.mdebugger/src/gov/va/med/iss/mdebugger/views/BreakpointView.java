...
import gov.va.med.iss.mdebugger.views.AllValuesView.NameSorter;
import gov.va.med.iss.mdebugger.views.AllValuesView.ViewLabelProvider;
...
public class BreakpointView extends ViewPart {
	...
	lblEdit.setToolTipText("Enter the TAG^ROUTINE for the desired breakpoint.");
	...
	private static void setUp() {
		...
		MDebuggerSteps.addBreakpoint(txtEdit.getText() + ";" + txtValue.getText());
		txtEdit.setText("");
		txtValue.setText("");
	}
	...
	private void makeActions() {
		deleteSelected = new Action() {
			public void run() {
				...
				if (data.length() > 0) {
					MDebuggerSteps.removeBreakpoint(data);
				}
				updateView(stringVal);
				viewer.getTable().deselectAll();
			}
		};
		deleteSelected.setText("Delete Selected Breakpoint(s)");
		deleteSelected.setToolTipText("This will remove the currently selected Breakpoint(s)");
		...
		deleteAll.setText("Delete All Breakpoints");
		deleteAll.setToolTipText("This will remove all current breakpoints");
