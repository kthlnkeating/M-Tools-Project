...
public class RoutineLoadPage extends PropertyPage implements IWorkbenchPropertyPage {
	...
	private void initializeControls(IProject project) throws CoreException {	
		...
		this.addServerNameSubfolder.setSelection(MEditorPrefs.getAddServerNameSubfolder(project));
		int numChars = MEditorPrefs.getAddNamespaceCharsSubfolder(project);
	...
	public boolean performOk() {
		try {
			...
			if (this.projectRootScheme.getSelection()) {
				MEditorPrefs.setNewFileFolderScheme(project, NewFileFolderScheme.PROJECT_ROOT);
			} else if (this.namespaceSpecifiedScheme.getSelection()) {
				MEditorPrefs.setNewFileFolderScheme(project, NewFileFolderScheme.NAMESPACE_SPECIFIED);
			} else {
				MEditorPrefs.setNewFileFolderScheme(project, NewFileFolderScheme.ASK);				
			}
			MEditorPrefs.setAddServerNameSubfolder(project, this.addServerNameSubfolder.getSelection());
			int numChars = Integer.parseInt(this.addNamespaceCharsSubfolder.getText());
			MEditorPrefs.setAddNamespaceCharsSubfolder(project, numChars);
		} catch (CoreException coreException) {
			StatusManager.getManager().handle(coreException, MEditorPlugin.PLUGIN_ID);
		}
