...
//			if (crSeen) //jspivey-- commenting out
	...
	private static void checkText() {
		if ( !(isTimerFired)) {
			if ( newText.compareTo("") != 0 ) {
				...
				//MDebuggerConsoleDisplay.updateView(currentText, false); --jspivey/commenting out
			}
		}
	}
	...
	private static void sendReadInput(String outString, boolean isTimerFired) {
		...
		if (! (myConnection == null)) {
			try {
				...
				if (e.getMessage() == "") {
						MDebuggerSteps.clearRepeatLastDebug(); //does this ever get called?--jspivey
				}
				else {
					e.printStackTrace();
