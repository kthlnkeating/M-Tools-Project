...
public class GTMSSHOutputStream extends OutputStream {
	private static String BREAK_IDENTIFIER = "At M source location";
	...
	private void auxWrite(byte b) throws IOException {
		if (this.state == OutputStreamState.NOT_CONNECTED) {
		...
		} else if (this.state == OutputStreamState.BREAK_SEARCH) {
	...
	private void auxWriteBreak(byte b, byte[] pattern) throws IOException {
		...
		for (int i=this.debugCount-1; i>=endIndex; --i) {
			if (epb == this.debugInfo[i]) {
				...
				this.debugCount = 0;
				this.state = OutputStreamState.BREAK_SEARCH;
				if (currentState == OutputStreamState.WAITING_PROMPT) {
	...
	private MStackInfo[] infoToStacksInfo(String info) {
		...
		for (String piece : pieces) {
			if (piece.indexOf(BREAK_IDENTIFIER) >= 0) break;
			++breakLineIndex;
		}
		...
		String entryInfo = pieces[breakLineIndex].split(BREAK_IDENTIFIER)[1].trim();
		MCodeLocation codeLocation = MCodeLocation.getInstance(entryInfo);

		MVariableInfo[] variableInfos = new MVariableInfo[breakLineIndex-1];
		int index = 0;
		for (int i=0; i<breakLineIndex-1; ++i) {
			String nv = pieces[i];
